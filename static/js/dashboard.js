...
function updateDoor(isOpen, time) {
    ...
    if (summary) summary.textContent = isOpen ? "Mở" : "Đóng";
    if (timeElement) timeElement.textContent = formatTime(time);

    if (isOpen) {
        value.textContent = "Đang mở";
        visual.textContent = "🔓";
        visual.style.background = "rgba(245,158,11,0.1)";
        status.textContent = "OPEN";
        status.className = "sensor-status warning";
    } else {
...
async function initializeDashboard() {
    ...
    setInterval(async () => {
        if (currentRoom) await Promise.all([loadSensors(), loadDevices()]);
    }, REFRESH_INTERVAL);

    setInterval(async () => {
        if (currentRoom) await loadTemperatureHistory();
    }, 15000);
}
