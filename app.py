...
from database import (
    lay_danh_sach_phong, lay_phong, lay_che_do_phong,
    lay_sensor_hien_tai, lay_sensor, lay_sensor_history,
    lay_device_hien_tai, lay_device,
    lay_danh_sach_hoc_vien, them_hoc_vien, sua_hoc_vien, xoa_hoc_vien,
    gan_the_hoc_vien, tim_hoc_vien_theo_card,
    lay_danh_sach_diem_danh, lay_thong_ke_diem_danh,
    lay_danh_sach_sessions, lay_bang_diem_danh_buoi_hoc,
    cap_nhat_diem_danh_thu_cong, dong_buoi_diem_danh,
    lay_danh_sach_lop_hoc_phan, lay_danh_sach_mon_hoc,
    lay_thoi_khoa_bieu, tao_session_moi, ghi_nhan_diem_danh_sinh_vien,
    tao_lich_hoc, sua_lich_hoc, xoa_lich_hoc
)
...
def api_add_student():
    ...
    return jsonify({"success": True, "message": "Them hoc vien thanh cong", "student_id": result}), 201
...
def api_create_session():
    ...
    success, session_id, message = tao_session_moi(
        room_id=room_id, class_id=int(class_id),
...
def api_upsert_student_session_record(session_id, student_id):
    """Cập nhật hoặc tạo mới bản ghi điểm danh cho sinh viên trong buổi học."""
    if not request.is_json:
        return jsonify({"success": False, "message": "Content-Type phải là application/json"}), 400
    ...
    success, message = ghi_nhan_diem_danh_sinh_vien(
        ...
        status=new_status, method="MANUAL_TEACHER", note=note
    )
    if not success:
        return jsonify({"success": False, "message": message}), 400
...
def api_export_session_csv(session_id):
    ...
    status_map = {
        "PRESENT": "Có mặt (Đúng giờ)", "LATE": "Đi muộn",
        "ABSENT_EXCUSED": "Nghỉ có phép", "ABSENT_UNEXCUSED": "Vắng không phép"
    }
    method_map = {
        "RFID": "Quẹt thẻ RFID", "MANUAL_TEACHER": "Giảng viên xác nhận", "AUTO_ABSENT": "Hệ thống tự động"
    }
    ...
    for idx, r in enumerate(records, start=1):
        writer.writerow([
            idx, r["student_code"], r["full_name"], r["class_name"] or "",
            r["card_uid"] or "", status_map.get(r["status"], r["status"]),
            r["checkin_time"] or "", method_map.get(r["method"], r["method"]), r["note"] or ""
        ])
...
def api_create_schedule():
    ...
    try:
        success, new_id, message = tao_lich_hoc(
            class_id=int(class_id), room_id=room_id, day_of_week=int(day_of_week),
            start_time=start_time, end_time=end_time, late_grace_period_mins=int(late_grace)
        )
        if not success:
            return jsonify({"success": False, "message": message}), 400
...
def api_update_schedule(schedule_id):
    ...
    try:
        success, message = sua_lich_hoc(
            schedule_id=schedule_id, class_id=int(class_id), room_id=room_id,
